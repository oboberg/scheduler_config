import lsst.ts.schedulerConfig.instrument.rotator
assert type(config)==lsst.ts.schedulerConfig.instrument.rotator.Rotator, 'config is of type %s.%s instead of lsst.ts.schedulerConfig.instrument.rotator.Rotator' % (type(config).__module__, type(config).__name__)
